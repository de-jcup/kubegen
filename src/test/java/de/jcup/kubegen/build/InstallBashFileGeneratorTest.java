...
public class InstallBashFileGeneratorTest {
    ...
    @Test
    void null_key_defined_throws_exception_on_construction() {
        assertThrows(IllegalArgumentException.class, () -> new InstallBashFileGenerator(null));
    }

    @Test
    void empty_namespace_defined_throws_exception_on_construction() {
        BuildCommand data = new BuildCommand();
        data.k8sNamespace = "";

        assertThrows(IllegalArgumentException.class, () -> new InstallBashFileGenerator(data));
    }

    @Test
    void generate_simple_install_shellscript_and_check_content_namespace_set() throws Exception {
        /* prepare */
    ...
    @Test
    void generate_simple_install_shellscript_and_check_content__only_context_set() throws Exception {
        /* prepare */
        Path projectFolder = Files.createTempDirectory("kubegen-project");
        Path templateFolder = projectFolder.resolve("templates");
        Path targetFolder = Files.createTempDirectory("kubegen-target");
    ...
    @Test
    void generate_simple_install_shellscript_and_check_content__namespace_set_context_set_and_kubeconfig_set()
            throws Exception {
        /* prepare */
        Path projectFolder = Files.createTempDirectory("kubegen-project");
        Path templateFolder = projectFolder.resolve("templates");
        ...
        /* test */
        String result = Files.readString(targetFolder.resolve("install.sh"));
        String apply1 = "kubectl --kubeconfig=\"cube_configfile\" --context=\"context_name\" --namespace=\"namespace_name\" apply -f "
                + path1.toAbsolutePath().toString();
        String apply2 = "kubectl --kubeconfig=\"cube_configfile\" --context=\"context_name\" --namespace=\"namespace_name\" apply -f "
                + path1.toAbsolutePath().toString();
